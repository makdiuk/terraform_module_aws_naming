...
resource "null_resource" "label" {
  triggers = {
    ...
    short_function_name = lower(join("", compact(concat(tolist([null_resource.short_label.triggers.short_function, null_resource.short_label.triggers.short_solution, null_resource.short_label.triggers.short_environment, null_resource.short_label.triggers.short_region, var.alteration])))))
